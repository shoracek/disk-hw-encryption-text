...
Some of those features are locking ranges, allowing to deny access to only part of the disk, authorities, allowing to specify a set of capabilities to credentials, or shadow MBR, to allow pre-boot authentication.
...
\begin{itemize}
\item The \emph{Properties} method enables one to find a compromise between the communication capabilities of the host and the TPer or simply find the communication capabilities of the TPer. Using this method, the properties defined earlier in section~\ref{TODO}, such as maximum packet size, are established.
\item The \emph{StartSession} and \emph{SyncSession} methods provide a way to start an unencrypted session.
...
\item The \emph{StartTrustedSession} and \emph{SyncTrustedSession} methods provide a way to complete challenge-response authentication or to set up secure messaging/key exchange.
\item The \emph{CloseSession} method used to close the session.
\end{itemize}
...
\begin{itemize}
\item \emph{Nonexistent} state is describes SP which does not exist. The SP might not have been created or already deleted.
...
\item \emph{SingleUser Feature Descriptor} reports information about SingleUser Feature Set, such as whether it is activated,
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \emph{MBRControl} table metadata for MBR shadowing (Enable, Done, DoneOnReset).
    ...
    \begin{itemize}
        \item \emph{DataStore} byte table for storage of arbitrary data
    \end{itemize}
\end{itemize}
...
\label{subsection:opal_opal_ioctl}
...
A short program showcasing the usage of this interface can be seen in the appendix~\ref{TODO}.
