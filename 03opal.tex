...
% \todo[color=green]{maybe describe tokens? but that seems like a little bit too much,,, right?, maybe just simple, sequence, control, added lexical to make it obviious, good enought i guess}
...
\bitbox{1}{\small Start List token} \\
\bytefieldsetup{bitheight=2.5ex}%
\wordbox[lrt]{1}{\vspace{3mm} arguments} \\
\bytefieldsetup{bitheight=0ex}%
\skippedwords \\
\bytefieldsetup{bitheight=2.5ex}%
\wordbox[lrb]{1}{} \\
...
\begin{tikzpicture}[node distance={20mm}, main/.style = {draw, rectangle}, edge/.style = {}]
% \node[main] (M) [below left=10mm and 5mm of MI] {Manufactured};
\node[main] (M) {Manufactured};
\node[main] (MI) [above=10mm of M] {Manufactured-Inactive};
\node[main] (MX) [below=10mm of M] {Manufactured-Failed};
\node[main] (MDF) [left=10mm and 5mm of M] {Manufactured-Disabled-Frozen};
\node[main] (MF) [below=10mm of MDF] {Manufactured-Frozen};
\node[main] (MD) [above=10mm of MDF] {Manufactured-Disabled};

\draw[edge,->] (MI) -- (M);

\draw[edge,->] (M) edge [loop right] (M);
\draw[edge,->] (M) edge (MX);
\draw[edge,<->] (M) edge (MF);
\draw[edge,<->] (M) -- (MD);

\draw[edge,<->] (MD) -- (MDF);
% \draw[->] (MD) edge [bend right] (MX);
\draw[edge,-] (MD) -> (-8.5, 1.6);

% \draw[->] (MDF) edge [bend right] (MX);
\draw[edge,->] (MDF) -> (-8.5, 0);

% \draw[->] (MF) -- (MX);
\draw[edge,->] (MF) -> (-8.5, -1.6);

\draw[edge,-] (-8.5, 1.6) edge (-8.5, -2.2);
\draw[edge,-] (-8.5, -2.2) edge (0, -2.2);
\draw[edge,->] (0, -2.2) -> (MX);
...
% The Opal standard does not require the presence of every table defined by the templates. Similarly to the methods, only a selection of tables is required to be implemented.
...
\begin{itemize}
    \item \emph{TPerInfo} table contains metadata about the TPer, such as its version, implemented SSCs, or free space for issuance.
    \item \emph{Template} table contains metadata about templates, such as the number of their instances.
    \item \emph{SP} table contains metadata about SPs, such as their current life cycle or frozen status.
\end{itemize}
\begin{samepage}
    \item Opal additional tables:
\begin{itemize}
\item \emph{DataRemovalMechansim} table describes the mechanism used for data removal, such as overwrite erase, block erase, or cryptographic erase.
\end{itemize}
\end{samepage}
\end{itemize}
...
Locking SP shares the Base Template and Crypto Template methods with Admin SP. However, it also additionally provides the Locking Template methods.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \emph{LockingInfo} table contains metadata for locking mechanisms, such as the maximum number of locking ranges or supported types of encryption. 
        \item \emph{Locking} table contains the state of locking ranges, more closely described in Subsection~\ref{locking_range}.
        \item \emph{MBRControl} table contains metadata for MBR shadowing, such as Enable, Done, and DoneOnReset values.
        \item \emph{MBR} byte table contains the shadow MBR.
...
\todo{maybe specify reset types separately, in separate section, rn they are defined in the locking range section, I think it's fine???}
...
\item \verb|ACTIVATE_LSP| command changes the state of Locking SP from Manufactured-Inactive to Manufactured state, enabling its usage. Additionally, it can also facilitate the setup of Single User Mode. \todo[color=green]{write about TPer states in the generic chapter. }
...
\item \verb|ADD_USR_TO_LR| command sets the ACE for locking and unlocking to the designated user. Contrary to the name, this request does not actually add a user and instead replaces any existing one with the new one. % adds a user to the locking range ACE. ...\verb|l_state == OPAL_RW| means writing ace...  (TODO: they seem to set it to "\verb#(user_uid || user_uid)#"??? why? bug? -> MRE is \verb|opal_util_linux.c|, only used last assigned to locking range can control it, does (not) work on all test disks
...
% This method not only destroys the data in the locking range but also resets the assigned user authority of the locking range and the user's password. 
...
\todo[color=green]{where to find the instructions?, \url{https://wiki.archlinux.org/title/Self-encrypting_drives}}

% Note that some of the the tools use ATA TRUSTED commands for managing ATA Opal devices. When using these commands,
